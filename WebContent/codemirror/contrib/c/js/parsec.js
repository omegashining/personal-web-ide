...
var JSParser = Editor.Parser = (function() {
  ...
  function CLexical(indented, column, type, align, prev, info) {
  ...
  function parseC(input, basecolumn) {
    // Wrap the input in a token stream
    var tokens = tokenizeC(input);
    ...
    var lexical = new CLexical((basecolumn || 0) - indentUnit, 0, "block", false);
    ...
    function next(){
      ...
      if (token.content == "\n"){
        ...
        // them.
        token.indentation = indentC(lexical);
      }
      // No more processing for meaningless tokens.
      if (token.type == "whitespace" || token.type == "comment" || token.type == "import")
        return token;
    ...
    function copy(){
      ...
      return function copyParser(input){
        ...
        column = indented = 0;
        tokens = tokenizeC(input, _tokenState);
        return parser;
      };
    }
    ...
    function pushlex(type, info) {
      var result = function(){
        lexical = new CLexical(indented, column, type, null, lexical, info)
      };
      result.lex = true;
      return result;
    }
    ...
    function property(type){
      if (type == "variable") {mark("c-property"); cont();}
    }
    // This parses a property and its value in an object literal.
    function objprop(type){
      if (type == "variable") mark("c-property");
